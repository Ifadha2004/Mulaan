...
// UK dress sizing, matching the brand's size guide chart
...
export default function VariantManager({ value, onChange, productSlug }: VariantManagerProps) {
  ...
  const addSizeToColor = (color: string) => {
    const existingSizesInColor = groups[color]?.map((g) => g.variant.size) || []
    const nextSize = UK_SIZES.find((s) => !existingSizesInColor.includes(s)) || UK_SIZES[0]
    const skuCount = value.length + 1
  ...
  const addNewColor = () => {
    ...
    onChange([
      ...value,
      { size: UK_SIZES[2], color: trimmed, stock: 0, sku: `${skuPrefix}-${skuCount}` },
    ])
    setNewColorName('')
  }
  ...
  return (
    <div>
            ...
            <div key={color} className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              <div className="p-3">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-gray-400 text-xs">
                      <th className="pb-2 pr-3 font-medium w-28">Size (UK)</th>
                      <th className="pb-2 pr-3 font-medium w-24">Stock</th>
                      <th className="pb-2 pr-3 font-medium">SKU</th>
                      <th className="pb-2 w-8"></th>
                    </tr>
                    ...
                    {groups[color].map(({ variant, index }) => (
                      <tr key={index} className="border-t border-gray-100 first:border-0">
                        <td className="py-1.5 pr-3">
                          <input
                            type="text"
                            list="uk-size-options"
                            value={variant.size}
                            onChange={(e) => updateVariant(index, 'size', e.target.value)}
                            placeholder="e.g. 12"
                            className="w-24 px-2 py-1.5 border border-gray-300 rounded focus:ring-2 focus:ring-brand-green-800 outline-none"
                          />
                        </td>
                        <td className="py-1.5 pr-3">
                          <input
      ...
      {/* Shared suggestion list for the size inputs above — lets you type a UK
          size quickly via autocomplete, while still allowing free text like
          "One Size" for products that don't use dress sizing. */}
      <datalist id="uk-size-options">
        {UK_SIZES.map((s) => (
          <option key={s} value={s} />
        ))}
      </datalist>
