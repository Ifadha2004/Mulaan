...
import NewsletterSignup from '@/components/customer/shared/NewsletterSignup'

// Simple TikTok icon since lucide-react doesn't ship one
...
export default function Footer() {
  ...
  const footerLinks = {
    ...
    socials: [
      { name: 'Instagram', href: 'https://www.instagram.com/mulaan.lk', external: true },
      { name: 'TikTok', href: 'https://www.tiktok.com/@mulaan.lk', external: true },
      { name: 'WhatsApp', href: 'https://wa.me/94760100965', external: true },
    ],
    legal: [
      ...
      <div className="container mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-12 relative z-10">
        ...
        <div className="md:col-span-3 space-y-6">
          <div className="flex items-center gap-4">
            ...
            <span
              className="text-2xl tracking-[0.2em] text-brand-cream uppercase font-serif italic"
            ...
            <ul className="space-y-3">
              {footerLinks.pages.map((link) => (
                <li key={link.name}>
                  <Link
                    href={link.href}
                    className="text-brand-cream/60 hover:text-brand-gold text-xs transition-all duration-500 font-light"
                  >
                    {link.name}
                  </Link>
                </li>
              ...
              {footerLinks.socials.map((link) => (
                <li key={link.name}>
                  <a
                    ...
                    className="flex items-center gap-2 text-brand-cream/60 hover:text-brand-gold text-xs transition-all duration-500 font-light"
                  >
                    {link.name === 'Instagram' && <Instagram className="w-3.5 h-3.5" />}
                    {link.name === 'TikTok' && <TikTokIcon className="w-3 h-3" />}
                    {link.name === 'WhatsApp' && <MessageCircle className="w-3.5 h-3.5" />}
                    {link.name}
                  </a>
                </li>
              ))}
